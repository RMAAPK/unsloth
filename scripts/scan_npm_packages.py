...
def parse_lockfile(path: Path) -> tuple[list[PackageEntry], list[Finding]]:
    """Return (entries, structural_findings). Structural findings are HIGH-severity refusals that short-circuit the scan (e.g. non-registry resolved URLs), surfaced here so this scanner is standalone-runnable."""
...
def safe_extract(
    ...
    *,
    ...
    """Extract tarball_path under extract_root with policed members, returning None on success or a string describing the refusal. Streams via `r|gz` so we can abort mid-extraction without having materialised the rest of the archive."""
...
def _bracket_lr(line: str) -> tuple[int, int]:
    """Order-aware bracket reduction of one already-string-blanked line: ``(L, R)``, where ``L`` counts closers with no opener earlier on the line and ``R`` counts openers with no closer later. A plain net count collapses order and masks a trailing opener that follows leading closers (``}); const opts = {`` nets -1 and hides the ``{`` that opens the host-config object), so tracking the running minimum keeps that opener visible. Only bracket characters are walked so a long minified line stays cheap."""
    depth = 0
    low = 0
...
def _find_unescaped(line: str, quote: str, start: int) -> int:
    """Index of the next ``quote`` at or after ``start`` not escaped by a backslash, or -1. Skips ``\\x`` pairs so an escaped quote inside the string is ignored."""
    i, n = start, len(line)
    while i < n:
        if line[i] == "\\":
            i += 2
...
def _index_text(text: str) -> tuple[list[str], list[str], list[str], list[int]]:
    """Precompute once per evidence call: raw lines for display, two string-blanked views for bracket counting (single-line via regex, and multi-line aware so a template literal spanning lines is blanked), and newline offsets for O(log n) offset-to-line mapping. Avoids re-splitting and re-counting the whole file on every match, which was O(matches x file size)."""
    lines = text.split("\n")
...
def _canon_preserve_strings(text: str) -> str:
    """Whitespace canon that collapses runs OUTSIDE string literals to a single space (so a reindent stays stable) while preserving whitespace INSIDE single/double/backtick literals (so a changed payload body reopens). A plain ``" ".join(text.split())`` erases both, suppressing an intra-literal payload edit along with harmless indentation. Used for the evidence hash and the logical-line digests so the two stay consistent."""
    out: list[str] = []
    i, n = 0, len(text)
    quote: str | None = None
    pending_space = False
...
def _logical_line_text(
    ...
    """The matched line plus the bracket group it belongs to, so a changed ``path``/``headers``/body on another line binds. Returns the UNION of the groups found in the single-line-blanked view (a payload embedded inside a template still counts) and the multi-line-blanked view (a bracket inside a multi-line template no longer closes the group early); unioning never shrinks the span below either view."""
...
def _stream_overflow_digest(
    ...
    """A single digest binding the LOGICAL line (the bound bracket-group context, not just the regex match text) of every overflow match in the iterable, plus the count folded. Streams the matches so a huge overflow never materializes a list. Whitespace-normalized to match _evidence_hash so a reindent does not reopen."""
    h = hashlib.sha256()
    count = 0
    for m in matches:
...
def _fold_overflow_match(
    ...
    """Fold one overflow match's whitespace-normalized logical-line context into the running hash ``h``. Shared by _stream_overflow_digest and the inline overflow fold in _outbound_host_evidence so both produce the identical digest."""
...
def _ioc_evidence(text: str, needle: str) -> str:
    """Matched-line context (with bracket-group continuation) for a literal IOC needle, so a changed adjacent fetch/exfil body reopens the key instead of riding the bare constant. Falls back to the needle itself if, defensively, nothing matches."""
...
def _outbound_host_evidence(text: str, host: str) -> str:
    """Evidence capturing the host WITH its outbound context (URL path, fetch call, host config), so a changed path/headers/body reopens the key instead of riding the bare host literal. Falls back to the host if none matches."""
    host_re = re.escape(host)
    patterns = (
...
def _norm_pkg_name(display: str) -> str:
    """``@scope/pkg@1.2.3`` / ``pkg@1.2.3`` -> name without the version. The version is the LAST ``@``-separated field; a leading ``@`` (scope) is preserved. Lower-cased, since npm names are case-insensitive. Sentinels like ``<root>`` pass through unchanged."""
    s = (display or "").strip()
    at = s.rfind("@")
...
def _relpath_in_package(filename: str) -> str:
    """Path within the published package, stable across version bumps. npm tarballs root every file at ``package/``; strip it so the key is the real source path and a new file with the same basename in a different directory is not silently suppressed."""
...
def _evidence_hash(evidence: str) -> str:
    """Stable digest of the matched evidence. The npm snippet carries no line markers, so it is already version-stable; whitespace outside string literals is collapsed (reindent-stable) while whitespace inside literals is preserved, so a changed payload body reopens but a formatter reindent does not."""
...
def _finding_key(f: Finding) -> tuple[str, str, str, str]:
    """Allowlist key: normalized package, package-relative path, pattern, and a hash of the matched evidence, so changed flagged code under an already-listed package/path/pattern reopens instead of riding the reviewed entry."""
    return (
